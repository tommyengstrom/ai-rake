module Rake.PostgresPoolSpec where
    ...
    if not postgresAvailable
        then
            ...
            describe "PostgreSQL Connection Pooling" $ do
                ...
                describe "runRakeStoragePostgres with a pool" $ do
                ...
                describe "Connection pool behavior" $ do
                    it "handles concurrent operations efficiently" $ do
                        ...
                        let runStack :: Eff '[RakeStorage, WithConnection, Error ChatStorageError, Error RakeError, Time, IOE] Int -> IO (Either RakeError (Either ChatStorageError Int))
                            runStack =
                                runEff
                                    . runTime
